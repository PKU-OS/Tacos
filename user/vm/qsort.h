#ifndef __LIB_VM_QSORT_H
#define __LIB_VM_QSORT_H
...
static uint8 pick_pivot(uint8* buf, size_t size) {
    assert(size >= 1);
...
/* Checks whether the SIZE bytes in ARRAY are divided into an
   ...
   PIVOT. */
static int is_partitioned(const uint8* array, size_t size, uint8 pivot, size_t left_size) {
    size_t i;

    for (i = 0; i < left_size; i++)
        if (array[i] >= pivot) return 0;
...
static size_t partition(uint8* array, size_t size, int pivot) {
    size_t left_size = size;
    uint8* first = array;
    uint8* last = first + left_size;
...
/* Returns 1 if the SIZE bytes in BUF are in nondecreasing
   order, 0 otherwise. */
static int is_sorted(const uint8* buf, size_t size) {
    size_t i;
